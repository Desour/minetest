...
int LuaAreaStore::l_from_string(lua_State *L)
{
	...
	return deserialization_helper(L, o->as.get(), is);
}
...
#if USE_SPATIAL
	if (type == "LibSpatial") {
		as = std::make_unique<SpatialAreaStore>();
	} else
#endif
	{
		as = std::make_unique<VectorAreaStore>();
	}
}

LuaAreaStore::~LuaAreaStore() = default;
