...
```lua
{
    ...
    light_source = 0,
    -- Only available if item is node. (FIXME)
    tool_capabilities = {
    ...
    -- TODO: by default, some callbacks should be nil (even if set on server), others
...
```lua
{
    ...
    -- Client-only fields generated by the engine
    minimap_color = <ColorSpec>,
    -- Color of node used for minimap.
    has_on_construct = false,
    has_on_destruct = false,
    has_after_destruct = false,
    -- TODO: I'm not sure if these are serialized. I copied from CPCSM, which might do BS.
    rightclickable = false,
    -- Whether server nodedef has `on_rightclick` callback defined.
    -- FIXME: choose other name, e.g. server_has_on_rightclick?

    -- TODO: same as for itemdefs
    -- e.g. on_rightclick = rightclickable and function(...) send_interact("rightclick", ...) end or nil
